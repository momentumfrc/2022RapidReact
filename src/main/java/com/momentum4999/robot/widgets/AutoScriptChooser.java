...
public class AutoScriptChooser extends SendableChooser<MoCodeRuntime> {
	public AutoScriptChooser(RobotContainer robot) {
		super();

		String defaultScript = MoCode.INSTANCE.getDefaultScriptName();
		this.setDefaultOption(defaultScript, MoCode.INSTANCE.getRunnableScript(defaultScript));

		for (String scriptName : MoCode.INSTANCE.getLoadedScripts()) {
			this.addOption(scriptName, MoCode.INSTANCE.getRunnableScript(scriptName));
		}
